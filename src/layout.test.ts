...
describe("Layout", () => {
  ...
  it("should give a single window the full frame", () => {
    ...
    const frames = calculateFrames("123", frame)
    expect(frames).toEqual([{ window: "123", frame: frame }])
  })

  it("should split two windows equally", () => {
    const frame = { x: 0, y: 0, width: 100, height: 100 }
    const frames = calculateFrames(createGroup("123", "456"), frame)
  ...
  it("should support splitting a frame based on ratio", () => {
    const frame = { x: 0, y: 0, width: 100, height: 100 }
    const frames = calculateFrames(createGroup("111", "222", "group1"), frame, {
      group1: {
        ratio: 0.8,
